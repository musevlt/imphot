...
def fit_image_photometry(hst, muse, regions=None, fix_scale=None,
    ...
    if display or plotfile is not None or save:
        (muse_im, hst_im, muse_ft, hst_ft) = _generate_images(mfft, hfft,
                                                              sky_slice)
...
def _generate_images(mfft, hfft, sky_slice):
    """Generate images and FFTs that are related to the final best-fit
    parameters.
    ...
    hfft  : numpy.ndarray(dtype=complex)
       ...
       np.fft.rfft2().
    sky_slice : list of slices
       The boundaries of the region of the inverse FFT of mfft
       and hfft that corresponds to the original MUSE and HST images.

    Returns
    -------
    out : (muse_im, hst_im, muse_ft, hst_ft)
       The final MUSE and HST images, plus their full complex FFTs, which
       are centered at the center of the image (ie at [ny//2, nx//2]).
    ...
    return (muse_im, hst_im, muse_ft, hst_ft)
